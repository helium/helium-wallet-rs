...
async fn perform_add(
    ...
    if hotspot_issued || commit.commit {
        let tx = hotspot::dataonly::onboard(&client, subdao, &gateway, update, &keypair).await?;
...
impl MobileCommand {
    pub async fn run(&self, opts: Opts) -> Result {
        match self {
            Self::Txn(cmd) => cmd.run(opts).await,
...
struct MobileOnboard {
    ...
    /// Base64 encoded hotspot transaction.
    ///
    /// The gateway transaction is generated by the 'txn' command
    txn: Transaction,
