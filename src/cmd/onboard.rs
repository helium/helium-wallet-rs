...
impl Cmd {
    pub fn run(&self, opts: Opts) -> Result {
        ...
        match &mut envelope.txn {
            ...
            _ => bail!("Unsupported transaction for onboarding"),
        };
        ...
        let envelope = match envelope.payer()? {
            Some(key) if &key == wallet_key => match &mut envelope.txn {
                ...
                _ => Err(anyhow!("Unsupported transaction for onboarding")),
            },
            Some(_maker_key) => {
                if self.onboarding.is_none() {
                    Err(anyhow!("Staking server requires an onboarding key"))
                } else {
